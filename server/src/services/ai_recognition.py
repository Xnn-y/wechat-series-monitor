...
def build_account_list_prompt(standard_accounts: List[str]) -> str:
    ...
    return "\n".join([
        ...
        "The first account row below the title/header is the user's own account/self row. Skip exactly that first row.",
        "After skipping the first row, scan every remaining account row from top to bottom.",
        "Only return accounts visibly present in the following list, from top to bottom.",
        "Do not start from the middle of the list. Include the first visible standard account row after the skipped self row.",
        ...
        "The skipped first row may display '账号停止使用'. This row is not a target account.",
        "Ignore any account that is not in the allowed standard list.",
