...
// ============================================================
var TARGET = "关注";
var ROI = [0, 0.05, 1, 0.55];          // 上半区 [xRatio, yRatio, wRatio, hRatio]
var FALLBACK = [0.50, 0.245];           // 备用比例坐标，略高于消息栏
...
function main() {
    ...
    if (result.ocrResult) {
        ...
        for (var i = 0; i < items.length; i++) {
            var item = items[i];
            var b = item.bounds || {};
            console.log("  \"" + item.label + "\" @ " + b.left + "," + b.top + "-" + b.right + "," + b.bottom);
        }
    }

    toastLog("点击「关注」完成: " + result.source);
}
...
function ocrFindPoint(target, roi, fallback) {
    var result = {
        ok: false, clickPoint: null, source: "",
        ocrResult: null, matchItem: null, skippedReason: "",
        target: target, roi: roi, fallback: fallback
    };

    if (!requestScreenCapture()) { result.skippedReason = "请求截图权限失败"; return result; }
    sleep(300);

    var img = captureScreen();
    if (!img) { result.skippedReason = "captureScreen() 返回空"; return result; }
    ...
    try {
        ...
        if (region && (!result.ocrResult.available || result.ocrResult.count === 0)) {
            ...
            var m2 = _findText(result.ocrResultFull, target);
            if (m2) {
                result.ok = true; result.source = "ocr_fullscreen";
                result.clickPoint = _boundsCenter(m2.bounds, null);
                result.matchItem = m2;
                return result;
            }
        }
        ...
        if (fallback && fallback.length >= 2) {
            result.ok = true; result.source = "fallback";
            result.clickPoint = {
